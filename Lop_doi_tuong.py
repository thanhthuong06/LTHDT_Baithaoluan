...
class Reader(Person):
    ...
    def input_info(self):
        ...
        if self.borrowed_books:
            print(f"Sách đang mượn: {', '.join(self.borrowed_books)}")
        else:
            print("Sách đang mượn: Không có")
...
# BorrowSlip
class BorrowSlip:
    def __init__(self, borrow_id="", book_id="", reader_id="", borrow_date="", return_date="", quantity=0):
    ...
    def display_info(self):
        ...
        print(f"{'Mã phiếu':<15} | {'Mã sách':<8} | {'Mã độc giả':<10} | {'Ngày mượn':<10} | {'Ngày trả':<10} | {'Số lượng':<8}")
        print(f"{self.borrow_id:<15} | {self.book_id:<8} | {self.reader_id:<10} | {self.borrow_date:<10} | {self.return_date if self.return_date else 'Chưa trả':<10} | {self.quantity:<8}")
